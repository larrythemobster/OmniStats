...
std::string PairSpeed(float scope_v, float self_v, bool always_pair, const std::string& suffix, bool useImperialUnits) {
    ...
    if (useImperialUnits && is_speed) {
        ...
        if (display_suffix == " kph") display_suffix = " mph";
        else if (display_suffix == " KPH") display_suffix = " MPH";
        else if (display_suffix == "kph") display_suffix = "mph";
        else if (display_suffix == "KPH") display_suffix = "MPH";
    }
...
std::string RankTier(const std::string& tier, bool useRomanNumerals) {
    ...
    std::string res = tier;
    size_t pos;
    if ((pos = res.find("Div IV")) != std::string::npos) res.replace(pos, 6, "Div 4");
    else if ((pos = res.find("Div III")) != std::string::npos) res.replace(pos, 7, "Div 3");
    else if ((pos = res.find("Div II")) != std::string::npos) res.replace(pos, 6, "Div 2");
    else if ((pos = res.find("Div I")) != std::string::npos) res.replace(pos, 5, "Div 1");
